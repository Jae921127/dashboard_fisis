...
def build_terms_map(master_df: pd.DataFrame,
                    ...
                    sleep_sec=0.15) -> pd.DataFrame:
    """
    For each list_no (excluding `exclude`), try each term in `terms`.
    If fetch_info_schema_and_accounts returns non-empty (columns or accounts), we treat that term as 'working'.
    Saves a CSV with schema:
        listNo | terms
    where 'terms' is a space-joined string like "H Y".
    """
    records = []
    for _, rec in master_df.iterrows():
        ...
        working = []
        for t in terms:
            cols, accts = fetch_info_schema_and_accounts(
                list_no=ln, term=t,
        ...
        term_str = " ".join(working) if working else ""
        records.append({"listNo": ln, "terms": term_str})

    out = pd.DataFrame(records, columns=["listNo", "terms"]).sort_values("listNo").reset_index(drop=True)
