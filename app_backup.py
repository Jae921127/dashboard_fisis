...
from dash import (Dash, dcc, html, Input, Output, State, ALL, MATCH,
                  no_update, callback_context, DiskcacheManager)
...
# --- 1. SETUP ---
# Use diskcache for background callback results
cache = diskcache.Cache("./cache")
background_callback_manager = DiskcacheManager(cache)

# Get the secret API key from settings
API_KEY = DEFAULTS.get("api_key")

# --- 2. HELPER FUNCTIONS ---

def _bg_cache_key():
    # Must be top-level so inspect.getsource can read it
    return "_on_run_cache_key"
...
def create_login_layout():
    """Returns the HTML layout for the login screen."""
    return html.Div(className="login-container", children=[
        html.Div(className="login-box", children=[
            html.H2("API Key Authentication"),
            dcc.Input(id="api-key-input", type="password", placeholder="Enter your API key", className="login-input"),
            html.Button("Submit", id="submit-api-key", n_clicks=0, className="login-button"),
...
def create_app():
    ...
    # --- Build the entire main application layout ONCE and store it in a variable ---
    ...
    main_application_layout = html.Div([
    ...
    # --- Set the ROOT layout to a simple container for auth + loading ---
    app.layout = html.Div([
        dcc.Store(id='auth-status-store', storage_type='session'),
        html.Div(id='page-content'),
        html.Div(id='loading-overlay', className='loading-overlay', style={'display': 'none'}, children=[
            dcc.Loading(type="circle", color="#FFFFFF", fullscreen=False),
            html.Div("Running...", id='progress-text', className='loading-text')
        ])
    ])
    ...
    # -- Auth Callbacks --
    @app.callback(Output('auth-status-store', 'data'), Output('login-error-message', 'children'), Input('submit-api-key', 'n_clicks'), State('api-key-input', 'value'), prevent_initial_call=True)
    def check_api_key(n_clicks, entered_key):
        if entered_key == API_KEY: return 'authenticated', ''
        else: return no_update, 'Invalid API Key. Please try again.'

    @app.callback(Output('page-content', 'children'), Input('auth-status-store', 'data'))
    def render_page_content(auth_status):
        if auth_status == 'authenticated':
            return main_application_layout
        else:
    ...
    @app.callback(
        ...
        running=[
            ...
            (Output("ft-run", "disabled"), True, False),
        ],
        progress=[Output("progress-text", "children")],
        inputs=[Input("ft-store-run-trigger", "data")],
        state=[State("ft-store-run-params", "data"), State("toplevel-tabs", "value"), State({"type": "inner-tabs", "group": ALL}, "value")],
        
        # --- ADD THIS LINE ---
        cache_by=[_bg_cache_key],

        prevent_initial_call=True,
        background=True
    )
    def _on_run(set_progress, trigger, params, active_toplevel_tab, active_inner_tabs):
